...
pilot_data = [zeros(np,ofdm_frames); psk_data ; zeros(np,ofdm_frames)];  % Pilot Insertion
%OFDM symbol
...
% Receiver
        ...
        demod_data(:,j) = pskdemod(fft_data(5:124),m);     % Removal of pilot and Demodulation 
    end
    ber(i) = sum(sum(demod_data~=data))/((N_sub-2*np)*ofdm_frames);
end
ber_theoretical = berfading(snr,'PSK',m,1);
figure(1);
...
ylabel('BER--->');
axis([0 20 10^-5 10^0]);
title ('Simulating BER of QPSK in RAYLEIGH channel using OFDM');
legend ('BER Theoretical(QPSK)','BER Simulated(QPSK)');